...
def get_context_statistics(
    ...
    """
    Get contextual statistics comparing user's context with dataset averages.
    Returns both global averages and filtered averages for comparison.
    """
    df = load_data()
    ...
    global_std = df[numeric_cols].std().to_dict()
    
    # Filtered averages based on user context
    filtered_df = df.copy()
    filter_applied = []
    
    if state:
        filtered_df = filtered_df[filtered_df['State_Name'].str.lower() == state.lower()]
        filter_applied.append(f"state={state}")
    if season:
        filtered_df = filtered_df[filtered_df['Season'].str.lower() == season.lower()]
        filter_applied.append(f"season={season}")
    if crop:
        filtered_df = filtered_df[filtered_df['Crop'].str.lower() == crop.lower()]
        filter_applied.append(f"crop={crop}")
    
    context_avg = filtered_df[numeric_cols].mean().to_dict() if len(filtered_df) > 0 else global_avg
    context_std = filtered_df[numeric_cols].std().to_dict() if len(filtered_df) > 0 else global_std
    context_count = len(filtered_df)
    
    # Yield distribution for filtered context
    yield_distribution = {
        "min": float(filtered_df['Yield_ton_per_hec'].min()) if len(filtered_df) > 0 else 0,
        "q25": float(filtered_df['Yield_ton_per_hec'].quantile(0.25)) if len(filtered_df) > 0 else 0,
        "median": float(filtered_df['Yield_ton_per_hec'].median()) if len(filtered_df) > 0 else 0,
        "q75": float(filtered_df['Yield_ton_per_hec'].quantile(0.75)) if len(filtered_df) > 0 else 0,
        "max": float(filtered_df['Yield_ton_per_hec'].max()) if len(filtered_df) > 0 else 0,
        "mean": float(filtered_df['Yield_ton_per_hec'].mean()) if len(filtered_df) > 0 else 0
    }
    
    return {
        "global_averages": global_avg,
        "global_std": global_std,
        "context_averages": context_avg,
